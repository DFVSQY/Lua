...
float get_color_filed_simple(lua_State *L, const char *var)
{
    ...
    Pushes onto the stack the value t[k], where t is the value at the given index. As in Lua, this function may trigger
    a metamethod for the "index" event. Returns the type of the pushed value.
    */
...
void set_color_field(lua_State *L, const char *index, int value)
{
    ...
    /*
    Does the equivalent to t[k] = v, where t is the value at the given index, v is the value at the top of the stack,
    and k is the value just below the top. This function pops both the key and the value from the stack. As in Lua, this
...
void set_color_field_simple(lua_State *L, const char *index, int value)
{
    ...
    This function pops the value from the stack. As in Lua, this function may trigger a metamethod for the "newindex"
    event.
    */
...
void test_set_global_color()
{
    ...
    ColorTable color_tables[] = {{"WHITE", MAX_COLOR, MAX_COLOR, MAX_COLOR},
                                 {"RED", MAX_COLOR, 0, 0},
                                 {"GREEN", 0, MAX_COLOR, 0},
                                 {"BLUE", 0, 0, MAX_COLOR},
                                 {NULL, 0, 0, 0}};
...
void test_cal_rect_area()
{
    ...
    在将message压入stack时，lua_pcall会调用第4个参数（0表示没有，其他表示在stack的位置）指定的‘message
    handler’(如果有的话)进行处理。
    */
...
{
    /*
    Pushes a C function onto the stack. This function receives a pointer to a C function and pushes onto the stack a Lua
    value of type function that, when called, invokes the corresponding C function. Any function to be callable by Lua
    must follow the correct protocol to receive its parameters and return its results
    */
...
static int counter(lua_State *L)
{
    ...
    In particular, the expression lua_upvalueindex(1) gives the pseudo-index of the first upvalue of the running
    function. Again, this pseudo-index is like any stack index, except that it does not live on the stack
    */
...
void test_c_proc()
{
    ...
    const char *fname = "learn\\lua\\lproc.lua";
    if (luaL_loadfile(L, fname) || lua_pcall(L, 0, 0, 0))
        error(L, "cannot run config file, error msg:%s", lua_tostring(L, -1));
